...
export const RELEASES_API_URL = 'https://api.github.com/repos/el-j/omni-cad/releases/latest';
      ...
      res.on('end', () => {
        try {
          ...
          if (
            ...
            !('tag_name' in json) || typeof (json as Record<string, unknown>).tag_name !== 'string' ||
            !('html_url' in json) || typeof (json as Record<string, unknown>).html_url !== 'string'
          ) {
            reject(new Error('Unexpected GitHub Releases API response shape'));
            return;
          }
          const { tag_name, html_url } = json as { tag_name: string; html_url: string };
