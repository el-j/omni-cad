...
async function dismissOmniCadSetupPopup(
  ...
  const actionButtons = [
    window.locator('button:has-text("Use Detected Paths")'),
    window.locator('button:has-text("Skip For Now")'),
    window.locator('button:has-text("Cancel")'),
  ];

  for (let attempt = 0; attempt < 6; attempt += 1) {
    const isPopupVisible = await popupText
      .isVisible({ timeout: 250 })
      .catch(() => false);
    const actionVisible = await Promise.all(
      actionButtons.map(async (button) =>
        button.isVisible({ timeout: 150 }).catch(() => false),
      ),
    );
    if (!isPopupVisible && !actionVisible.some(Boolean)) {
      return;
    }
...
export async function openOmniCadViewer(
  ...
  while (Date.now() < deadline) {
    await dismissOmniCadSetupPopup(window);
    ...
    if (await restartBtn.isVisible({ timeout: 1000 })) {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (await paletteInput.isVisible()) {
          ...
          const entry = window.locator(
            '.quick-input-list-entry:has-text("OmniCAD: Open Viewer")',
          );
