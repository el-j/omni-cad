...
function getSiteRoot(baseUrl: string): string {
  ...
  const match = `/${normalized}`.match(/^(.*\/)(?:channels|versions)\/[^/]+\/$/);
  if (match?.[1]) {
    return match[1];
  }
  return `/${normalized}`;
...
function extractActiveTarget(baseUrl: string, pathname: string): string | null {
  ...
  const candidate = withoutBase.startsWith('/') ? withoutBase.slice(1) : withoutBase;
  if (!candidate) {
    return null;
  }
...
function DocsVersionSelector() {
  ...
  useEffect(() => {
    ...
    fetch(metadataUrl, { cache: 'no-store' })
      ...
      .then((payload) => {
        ...
        const sortedTargets = Array.isArray(payload.targets) ? payload.targets : [];
        setTargets(sortedTargets);

        const detected = extractActiveTarget(siteRoot, window.location.pathname);
        if (detected) {
        ...
        if (payload.defaultTarget) {
          const fallback = sortedTargets.find((item) => normalizePathFragment(item.path) === normalizePathFragment(payload.defaultTarget || ''));
          if (fallback) {
  ...
  const options = useMemo(() => {
    return targets.map((target) => {
      ...
      const suffix = target.kind === 'channel' && target.version ? ` (${target.version})` : '';
      return {
        key,
        ...
        <nav className="fixed top-0 left-0 right-0 z-1000 bg-black/60 backdrop-blur-2xl border-b border-white/5">
          <div className="container mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <ul className="hidden md:flex items-center gap-8">
              ...
              <li><Link to="/roadmap" className="text-sm font-medium text-gray-400 hover:text-white transition-colors">Roadmap</Link></li>
              <li><DocsVersionSelector /></li>
              ...
              <li>
                <Link to="/docs" className="px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-sm font-bold transition-all">
                  Install →
                </Link>
              </li>
            </ul>
        ...
        <footer className="py-20 border-t border-white/5 bg-black">
          <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex items-center gap-8">
              ...
              <Link to="/docs" className="text-sm text-blue-400 hover:text-white transition-colors font-semibold">Docs</Link>
              <Link to="/roadmap" className="text-sm text-gray-500 hover:text-white transition-colors">Roadmap</Link>
