    ...
    <section id="engines" className="py-32 bg-zinc-950">
      <div className="container mx-auto px-6">
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto mb-20">
          ...
          <h2 className="text-4xl font-bold mb-6">Pick your language, OmniCAD does the rest</h2>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {engines.map((e, i) => (
            <div key={i} className={`group relative p-8 rounded-3xl bg-zinc-900/50 border ${e.borderColor} hover:bg-zinc-900 transition-all duration-300`}>
              <div className="flex items-center gap-4 mb-6">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ${e.colorClass}`}>
                  {e.icon}
                </div>
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    {e.ext.map(ext => (
                      <span key={ext} className="text-[10px] font-mono font-bold text-gray-500 bg-white/5 px-1.5 py-0.5 rounded border border-white/5">
                        {ext}
                      </span>
                    ))}
                  </div>
