...
- [x] Add export flow integration tests for success and failure UX
- [x] Add protocol compatibility tests for extension <-> webview message types
- [x] Add tests ensuring unsupported export actions report clear errors

### Acceptance Criteria

- [x] No untested message types in protocol contracts
- [x] Export UX has deterministic tested outcomes
...
- [x] Add engine capability e2e checks (supported vs unsupported format behavior)
- [x] Add config reload e2e checks for engine path changes

### Acceptance Criteria

- [x] E2E covers every non-placeholder engine path
- [x] E2E validates capability gating behavior

## Milestone D: CI and Flakiness Controls (P2)

- [x] Define separate gates for unit/integration/e2e suites
- [x] Add retry policy only where deterministic flake is proven
- [x] Add fixture normalization and temp-file isolation
- [x] Keep runtime-heavy tests bounded by explicit timeout policy

### Acceptance Criteria

- [x] Test suite is stable across repeated CI runs
...
- [x] Missing test surfaces from audit are covered
- [x] No critical feature remains untested in at least one suitable layer
