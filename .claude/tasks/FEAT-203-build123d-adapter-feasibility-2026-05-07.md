...
- Implemented shared `PythonBrepAdapter` base class in `packages/extension/src/engines/PythonBrepAdapter.ts`.
  - Shared Python subprocess runner
  - Shared STL parsing (binary + ASCII)
  - Shared mesh-bound metadata computation
- Refactored `CadQueryAdapter` to extend `PythonBrepAdapter`.
- Implemented `Build123dAdapter` in `packages/extension/src/engines/Build123dAdapter.ts`.
  - Extension mapping: `.b3d.py`
  - First export contract: STL
  - Python 3.10+ runtime guard in runner
  - Missing-runtime guidance: `python3 -m pip install build123d`
- Router updates:
  - `.b3d.py` now routes to `build123d`
  - Compound extension resolver now supports `.cq.py` and `.b3d.py`
- Unit coverage added for build123d route and capability exposure.
- Validation:
  - `pnpm --filter omni-cad run pretest` ✅
  - `pnpm --filter omni-cad run test` ✅ (57 passing)
- Pending:
  - Add E2E story for `.b3d.py` render/export artifact assertion.
  - Extend export contract to STEP in follow-up slice.
